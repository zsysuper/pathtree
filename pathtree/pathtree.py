...
class Node(object):
    ...
    def Find(self, path):
        if len(path) == 0 or path[0] != "/":
            return None, None
        return self.find(self.splitPath(path))
    ...
    def FindPath(self, path):
        if len(path) == 0 or path[0] != "/":
            return None
        return self.find_path(self.splitPath(path))
    ...
    def DeleteLeaf(self, path, value):
        if len(path) == 0 or path[0] != "/":
            return None, None
        leaf, target_node = self.delete_leaf(self.splitPath(path), value)
    ...
    def DeletePath(self, path):
        if not path or path == "/" or path[0] != "/":
            ...
            return None, None
        target_node, father_node = self.delete_path(self.splitPath(path))
